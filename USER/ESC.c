...
uint16_t changeable_ccr = 20;//PWM占空比
        ...
        else if(HAL_GPIO_ReadPin(UOUT_GPIO_Port,UOUT_Pin) == GPIO_PIN_RESET)
        {
            ...
            if(OpenloopStart_Over_Flag == 1)
            {
                if(FirstChange_Flag == 1)
                {
                    FirstChange_Flag = 0;
                    tim3_counts = 0;
                    phase = ChangeStatus(U_W,changeable_ccr);//更新到U_V状态
                }
                else
                {
                    t_v_Data = SpeedDetection();//获取时间和速度数据
...
TimeAndSpeedData SpeedDetection(void)
{
    ...
    data.time = tim3_counts;
    
    if (data.time == 0)
    {
        data.speed = expectspeed;
    }
    else
    {
...
{
    if((tim3_counts - t_v_Data.time > (t_v_Data.time / 10)) && OpenloopStart_Over_Flag == 1)
    {
        Initialization();
